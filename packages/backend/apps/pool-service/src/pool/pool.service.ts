...
export class PoolService {
  ...
  async getOverview(): Promise<PoolOverviewResponseDto> {
    ...
    const overview: PoolOverviewResponseDto = {
      ...
      escrowFunded: (latest?.escrowFunded ?? 0n).toString(10),
      redemptionPrice: (latest?.redemptionPrice ?? 0n).toString(10),
      feeApyBps: (latest?.feeApyBps ?? 0n).toString(10),
      overdueFeeApyBps: (latest?.overdueFeeApyBps ?? 0n).toString(10),
      firstLossReserve: (latest?.firstLossReserve ?? 0n).toString(10),
      lpShareBps: (latest?.lpShareBps ?? 0n).toString(10),
      platformShareBps: (latest?.platformShareBps ?? 0n).toString(10),
      rebateShareBps: (latest?.rebateShareBps ?? 0n).toString(10),
      pendingAdmin: latest?.pendingAdmin ?? null,
      aprPct,
      downPaymentSharePct,
      poolPortionSharePct,
